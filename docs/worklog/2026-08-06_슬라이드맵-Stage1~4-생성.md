# 작업일지 — 2026-08-06 (담당: KimYunjae) · 로컬 변경 커밋 + 기획 슬라이드(3~6) → Stage1~4 맵 씬 생성

Ricochet-Apocalypse · Unity 6000.3.18f1 · 2D · Legacy Input Manager · URP · NavMeshPlus(2D NavMesh) · 2D Tilemap Extras(RuleTile)

이번 작업: (1) 로컬에 쌓여 있던 변경(기획 PPTX, 테스트 씬 Stage6/7·YJ 테스트씬, 리소스/메타 정리)을 커밋, (2) 기획 문서 **`docs/맵 임시파일(변경가능, 피드백 환영) (3).pptx`** 의 3~6번째 슬라이드를 참고해 **맵 에디터 인프라(MapTileLayers/GridModule)로 Stage1~Stage4 맵 씬을 생성**.
...
사용자 결정/해석: 슬라이드 3~6(4장) → `Assets/Scenes/Stage1~Stage4`(기존 `StageN` + Build Settings 규칙과 일치, 충돌 없음). 문서는 "임시/변경가능/피드백환영"이므로 적 타입 매핑·발사(스폰) 위치 등은 기본값으로 두고 피드백 대상으로 명시.
...
| 씬 | 슬라이드 | 제목 | 크기(W×H) | 적 |
|----|----|------|------|----|
| Stage1 | 3 | 쉬움(각 튜토리얼) | 9×15 | 3 |
| Stage2 | 4 | 쉬움(풀숲 튜토리얼) | 9×15 | 2 |
| Stage3 | 5 | 쉬움(속성 튜토리얼) | 9×15 | 3 |
| Stage4 | 6 | 어려움(12×20) 맵 1 | 12×20 | 5 |
